...
describe("Reserve", function () {
    ...
    describe("withdrawTo", function () {
        beforeEach(async () => {
            await trend.transfer(reserve.address, reserveBalance)
        })
        ...
        it("should revert if not exceed unlock time", async function () {
            await expect(reserve.withdrawTo(receiver.address, reserveBalance))
                .to.be.revertedWith("Reserve: Cannot Trade")
        });
        ...
        it("should withdraw correctly", async function () {
            await network.provider.send("evm_increaseTime", [oneWeek * 24])
            expect(await trend.balanceOf(reserve.address)).to.be.equal(reserveBalance)
            await reserve.withdrawTo(receiver.address, reserveBalance)
            expect(await trend.balanceOf(reserve.address)).to.be.equal(0)
        });
    })
    describe("Combined with contract marketplace", function () {
        it("should withdraw correctly with fee from marketplace", async function () {
            ...
            marketplace = await Marketplace
                .deploy(trendivision.address, defaulFeeDecimal, defaulFeeRate, feeRecipientAddress)
            ...
            await network.provider.send("evm_increaseTime", [oneWeek * 24])
            expect(await trend.balanceOf(reserve.address)).to.be.equal(reserveBalanceFee)
